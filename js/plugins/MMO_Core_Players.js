...
(function () {
  ...
  MMO_Core.socket.on('map_joined', function (data) {
    data.playerData.skin = JSON.parse(data.playerData.skin);
    if (
      MMO_Core_Players.Players[data.id] !== undefined &&
      $gameMap._events[MMO_Core_Players.Players[data.id]['_eventId']] !== undefined
    )
      $gameMap.eraseEvent(MMO_Core_Players.Players[data.id]['_eventId']);

    MMO_Core_Players.Players[data.id] = $gameMap.createNormalEventAt(
      data['playerData']['skin']['characterName'],
      data['playerData']['skin']['characterIndex'],
      data['playerData']['x'],
      data['playerData']['y'],
      2,
      0,
    ...
    MMO_Core_Players.Players[data.id].headDisplay = MMO_Core_Players.Players[data.id]
      .list()
      .push({ code: 108, indent: 0, parameters: ['<Name: ' + data['playerData']['username'] + '>'] });
  ...
  MMO_Core.socket.on('map_exited', function (data) {
    ...
    //Orphus - Animax remove parts when exited map
    let char = MMO_Core_Players.Players[data];
    // console.log('Animax remove parts when exited map ', char);
    char.clearXAnimParts();

    //Orphus - We need to wait for the event to be removed from the map before we can create a new one
    setTimeout(function () {
