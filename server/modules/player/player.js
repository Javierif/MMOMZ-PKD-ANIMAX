  ...
  io.on('connect', function (client) {
    ...
    client.on('player_dead', function () {
      ...
      client.emit('player_respawn', {
        mapId: MMO_Core.database.SERVER_CONFIG.newPlayerDetails.mapId,
        x: MMO_Core.database.SERVER_CONFIG.newPlayerDetails.x,
        y: MMO_Core.database.SERVER_CONFIG.newPlayerDetails.y,
      });
    });

    //ANIMAX SECTION

    client.on('animaX:changePart', function (payload) {
      ...
      client.broadcast.to(client.lastMap).emit('animaX:changePart', payload);
    });

    client.on('animaX:removePart', function (payload) {
      if (client.playerData === undefined) return;
      if (MMO_Core.database.SERVER_CONFIG.offlineMaps[client.lastMap] !== undefined) return false;

      payload.id = client.id;

      client.broadcast.to(client.lastMap).emit('animaX:removePart', payload);
    });

    client.on('animaX:stopAnimationAction', function (payload) {
      if (client.playerData === undefined) return;
      if (MMO_Core.database.SERVER_CONFIG.offlineMaps[client.lastMap] !== undefined) return false;

      payload.id = client.id;

      client.broadcast.to(client.lastMap).emit('animaX:stopAnimationAction', payload);
    });

    client.on('animaX:playAnimationAction', function (payload) {
      if (client.playerData === undefined) return;
      if (MMO_Core.database.SERVER_CONFIG.offlineMaps[client.lastMap] !== undefined) return false;

      payload.id = client.id;

      client.broadcast.to(client.lastMap).emit('animaX:playAnimationAction', payload);
    });

    client.on('animaX:clearParts', function (payload) {
      if (client.playerData === undefined) return;
      if (MMO_Core.database.SERVER_CONFIG.offlineMaps[client.lastMap] !== undefined) return false;

      payload.id = client.id;

      client.broadcast.to(client.lastMap).emit('animaX:clearParts', payload);
    });
  });
